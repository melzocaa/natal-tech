...
// Define a data alvo: 25 de dezembro do ano atual
const now = new Date();
let countdownDate = new Date(now.getFullYear(), 11, 25, 0, 0, 0, 0); 
// mês 11 = dezembro (0 = janeiro)

// se já passou o natal deste ano, conta para o próximo
if (now > countdownDate) {
  countdownDate = new Date(now.getFullYear() + 1, 11, 25, 0, 0, 0, 0);
}
...
class Particle {
  ...
  reset(initial = false) {
    ...
    this.radius = Math.random() * 3.5 + 1; // diferentes tamanhos
    this.speed = Math.random() * 1.2 + 0.5;
    this.wind = Math.random() * 1.2 - 0.6; // vento suave
    this.opacity = Math.random() * 0.5 + 0.4; // flocos mais claros e mais fortes
    this.swing = Math.random() * 2 + 1; // intensidade do balanço
    this.angle = Math.random() * Math.PI * 2; // ponto inicial da curva
  }

  update() {
    this.y += this.speed;
    this.angle += 0.01; // movimento suave
    this.x += Math.sin(this.angle) * this.swing * 0.2 + this.wind * 0.3;
  ...
  draw() {
    ...
    ctx.shadowBlur = 8; // brilho suave no floco (muito bonito)
    ctx.fill();
...
async function loadProducts() {
  ...
  const mockProducts = [
    { name: 'Panetone Trufado', old_price: 129.90, new_price: 99.90, emoji: '🥮' },
    { name: 'Árvore de Natal (1.80m)', old_price: 269.90, new_price: 200, emoji: '🎄' },
    { name: 'Presente Surpresa', old_price: 159.90, new_price: 130, emoji: '🎁' },
    { name: 'Luzes de Natal (Pisca-Pisca) LED', old_price: 40.99, new_price: 30, emoji: '💡' },
    { name: 'Kit Decoração Bolas Natalinas', old_price: 103.90, new_price: 90, emoji: '🔮' }
  ];
